...
Copy the block below to `CLAUDE.md` at the root of the new repo, before any code is written.

Claude Code reads it automatically at the start of every session, so these rules apply to
work done weeks later in a fresh conversation — not just the session where they were stated.
That's the difference between rules that hold and rules that erode.
